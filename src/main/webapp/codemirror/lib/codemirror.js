...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    } catch (upyours) { } // anti-IE + gazillion iframes hack
    ...
    function onMouseDown(e) {
      ...
      // PN MOD: if clicked in the horizontal scrollbar
      if (e.e.offsetY > n.scrollHeight)
      {
          return e.stop();
      }
    ...
    function scrollIntoView(x1, y1, x2, y2) {
      ...
      // PN mod to allow any javastatic (noneditable) bits to offset 
      
      var $javastatics = $(wrapper).find("pre.javastatic");
      if ($javastatics.length != 0)
      {
          pt += $javastatics.eq(0).height();            
      }
    ...
    function updateGutter() {
      ...
      var height = (hText - hEditor < 2 ? hEditor : hText);
  
      if ($("pre.javastatic").length != 0)
      {
          var heightOffset = $("pre.javastatic").eq(0).height();          
          var padding = parseInt($("pre.javastatic").eq(0).css("padding"))*2;
          if (isNaN(padding)) padding = 6;
          height = height - heightOffset-padding-1;
      }      
    ...
    // PN MOD FOR PP
    // Modded to take into account whether we're in the #code-main div, which is
    // absolutely positioned and thus needs offsetting by the document.scrollTop
    function posFromMouse(e, liberal) {
      var scrollTop = 0;
      if ($(e.target()).closest("div.CodeMirror").parent().attr("id") == "code-main") scrollTop = $(document).scrollTop();
      var off = eltOffset(lineSpace),
          x = e.pageX() - off.left,
          y = e.pageY() - off.top - scrollTop; // PN MOD FOR PP
