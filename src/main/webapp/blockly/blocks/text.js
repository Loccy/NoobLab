...
Blockly.Blocks['text_concatenate'] = {
  /**
  ...
  init: function() {
    ...
    this.setMutator(new Blockly.Mutator(['concatenate_create_with_item']));
   ...
   */
  decompose: function(workspace) {
    var containerBlock = workspace.newBlock('concatenate_create_with_container');
    containerBlock.initSvg();
    var connection = containerBlock.getInput('STACK').connection;
    for (var i = 0; i < this.itemCount_; i++) {
      var itemBlock = workspace.newBlock('concatenate_create_with_item');
      itemBlock.initSvg();
      connection.connect(itemBlock.previousConnection);
      connection = itemBlock.nextConnection;
    }    
  ...
  updateShape_: function() {      
    ...
    for (var i = 0; i < this.itemCount_; i++) {
      if (!this.getInput('ADD' + i)) {
        var input = this.appendValueInput('ADD' + i);
        if (i == 0) {
          input.appendField(/*Blockly.Msg.LISTS_CREATE_WITH_INPUT_WITH*/ "concatenate");
        }
      }
    }
    // Remove deleted inputs.
...
Blockly.Blocks['concatenate_create_with_container'] = {
  /**
...
Blockly.Blocks['concatenate_create_with_item'] = {
  /**
